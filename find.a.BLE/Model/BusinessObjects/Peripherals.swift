...
actor Peripherals {
    private var activeList = [String: Peripheral]()
    private var prevActiveList = [String: Peripheral]()
    private var pendingList = [String: Peripheral]()
    private var interrogationResults = [String: BLEInterrogationResult]()
    private var hiddenList = [String: Peripheral]()
    private var prevHiddenList = [String: Peripheral]()
    let scannerResultDispatcher = ValueDispatcher<([String:Peripheral], [String:Peripheral])>()
    ...
    func clearActiveList() {
        activeList = [:]
        interrogationResults = interrogationResults.filter { if case .success(_, _, _) = $0.value { return true } else { return false } }
        update()
    }
    ...
    func ping(scanResult: BLEScannerResult) {
        ...
        var peripheral = Peripheral(id: id, deviceName: scanResult.name, manufacturer: "", model: "", isHidden: hiddenList[scanResult.uuid] != nil, rssi: Int(scanResult.signal)!, lastPing: NSDate().timeIntervalSince1970)
        func updatePeripheral() {
            if case let .success(deviceName, manufacturer, model) = interrogationResults[id] {
                peripheral.deviceName = deviceName.isEmpty ? (peripheral.deviceName.isEmpty ? model : peripheral.deviceName) : deviceName
