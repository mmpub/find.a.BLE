...
struct ScanResultCSVExporter: Transferable {
    ...
    let scanResults: [Peripheral]


    static var transferRepresentation: some TransferRepresentation {
        DataRepresentation(exportedContentType: .commaSeparatedText) { archive in
            archive.scanResults.reduce(into: ["name,id (temporary),rssi"])  { lines, scanResult in
...
class AppViewModel: SingleInstance, ObservableObject {
    ...
    @Published var namedScanResults = [Peripheral]()
    @Published var hiddenScanResults = [Peripheral]()
    @Published var unnamedScanResults = [Peripheral]()

    unowned let modelManager: ModelManager
    let settingsViewModel: SettingsViewModel

    var subscriptions = [AnyCancellable]()
    var hiddenList = [String: Peripheral]()
    var scanResults = [String: Peripheral]()
    ...
    @MainActor func updateScanResults() {
        ...
        func searchResultsFilter(_ item: Peripheral) -> Bool {
            true
        }

        typealias SortPredicate = @MainActor (Peripheral, Peripheral) -> Bool

        func nameSortPredicate(_ a:Peripheral, _ b:Peripheral) -> Bool {
            let aName = a.deviceName.lowercased()
            let bName = b.deviceName.lowercased()
            return aName == bName ? a.id < b.id : aName < bName
        }

        func signalStrengthSortPredicate(_ a:Peripheral, _ b:Peripheral) -> Bool {
            a.rssi == b.rssi ? nameSortPredicate(a, b) : a.rssi > b.rssi
        }

        func recentPingSortPredicate(_ a:Peripheral, _ b:Peripheral) -> Bool {
